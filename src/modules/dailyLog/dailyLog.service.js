...
async function validateEntries(entries, companyId) {
  ...
  for (const [i, e] of entries.entries()) {
    if (!e.projectId) throw Object.assign(new Error(`entries[${i}]: projectId is required`), { statusCode: 400 });
    if (!e.description?.trim()) throw Object.assign(new Error(`entries[${i}]: description is required`), { statusCode: 400 });

    if (e.moduleId && isValidId(e.moduleId)) {
      const mod = await Module.findOne({ _id: e.moduleId, companyId }).select("status");
      if (!mod) throw Object.assign(new Error(`entries[${i}]: Module not found`), { statusCode: 404 });
      if (mod.status !== "IN_PROGRESS")
        throw Object.assign(
          new Error(`entries[${i}]: Module must be IN_PROGRESS (currently ${mod.status})`),
          { statusCode: 400 }
        );
    }
  }
...
export const getDailyLogsService = async ({
  ...
  const [rawData, total] = await Promise.all([
    DailyLog.find(query)
      ...
      .populate("entries.moduleId", "title"),
  ...
  const data = rawData.map((log) => {
    ...
    obj.entries = (obj.entries || []).map((e) => ({
      ...
      moduleTitle: e.moduleId?.title || "",
      projectId: e.projectId?._id ?? e.projectId,
      moduleId: e.moduleId?._id ?? e.moduleId,
    }));
    return obj;
  });
